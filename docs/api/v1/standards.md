...
- Every response will carry an `X-Request-ID` once request tracing is implemented.
...
- Include the response request ID in support reports. Header generation and logging are implemented by the dedicated request-tracing ticket, not PF-005.
